	...
	class XmlFlowAnnotationSource : IFlowAnnotationSource
	{
			...
			if (_methods.TryGetValue (method, out var ann) && ann.ParameterAnnotations != null) {
				...
				foreach (var (ParamName, Annotation) in ann.ParameterAnnotations) {
					if (ParamName == paramName && firstAppearance) {
						...
						_context.LogWarning ($"There are duplicate parameter names for '{paramName}' inside '{method.Name}' in '{_xmlDocumentLocation}'", 2024);
					}
				}
			}
			...
			foreach (var attribute in attributes.ToArray ()) {
				if (attribute.attributeName == "System.Diagnostics.CodeAnalysis.DynamicallyAccessedMembers" && attribute.arguments.Count == 1) {
					if (attribute.arguments.Count == 0) {
						_context.LogWarning ($"DynamicallyAccessedMembers attribute was specified but no argument was proportioned", 2020);
						...
						foreach (var argument in attribute.arguments.ToArray ()) {
							...
							if (Enum.TryParse (argument, false, out result)) {
								...
								_context.LogWarning ($"Could not parse argument '{argument}' specified in '{_xmlDocumentLocation}' as a DynamicallyAccessedMemberTypes", 2021);
							}
						}
					} else {
						_context.LogWarning ($"DynamicallyAccessedMembers attribute was specified but there is more than one argument", 2022);
					}
				}
			}
			return DynamicallyAccessedMemberTypes.None;
		...
		private void ProcessAssemblies (LinkContext context, XPathNodeIterator iterator)
		{
			while (iterator.MoveNext ()) {
				...
				if (assembly == null) {
					_context.LogWarning ($"Could not resolve assembly {GetAssemblyName (iterator.Current).Name} specified in {_xmlDocumentLocation}", 2007);
					continue;
				}
		...
		void ProcessTypes (AssemblyDefinition assembly, XPathNodeIterator iterator)
		{
			while (iterator.MoveNext ()) {
				...
				if (type == null) {
					_context.LogWarning ($"Could not resolve type '{fullname}' specified in {_xmlDocumentLocation}", 2008);
					continue;
				}
		...
		void ProcessFieldSignature (TypeDefinition type, string signature, XPathNodeIterator iterator)
		{
			...
			if (field == null) {
				_context.LogWarning ($"Could not find field '{signature}' in type '{type.FullName}' specified in { _xmlDocumentLocation}", 2016);
				return;
			}
		...
		void ProcessMethodSignature (TypeDefinition type, string signature, XPathNodeIterator iterator)
		{
			...
			if (method == null) {
				_context.LogWarning ($"Could not find method '{signature}' in type '{type.FullName}' specified in '{_xmlDocumentLocation}'", 2009);
				return;
			}
		...
		void ProcessMethodChildren (TypeDefinition type, MethodDefinition method, XPathNodeIterator iterator)
		{
			...
			if (returnParameterAnnotations.Count == 1) {
				...
				_context.LogWarning ($"There is more than one return parameter specified for '{method.Name}' in '{_xmlDocumentLocation}'", 2023);
			}
