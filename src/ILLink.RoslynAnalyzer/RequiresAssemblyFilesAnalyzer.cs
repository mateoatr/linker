...
namespace ILLink.RoslynAnalyzer
{
	...
	public sealed class RequiresAssemblyFilesAnalyzer : RequiresAnalyzerBase
	{
		...
		public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
			ImmutableArray.Create (GeneratedDiagnostics.GetSupportedDiagnosticsOnType (typeof (RequiresAssemblyFilesAnalyzer)));
		...
		private protected override DiagnosticDescriptor RequiresDiagnosticRule => GeneratedDiagnostics.GetDiagnostic ("IL3002");

		private protected override DiagnosticDescriptor RequiresAttributeMismatch => GeneratedDiagnostics.GetDiagnostic ("IL3003");
		...
		protected override bool ReportSpecialIncompatibleMembersDiagnostic (OperationAnalysisContext operationContext, ImmutableArray<ISymbol> dangerousPatterns, ISymbol member)
		{
			if (member is IMethodSymbol && ImmutableArrayOperations.Contains (dangerousPatterns, member, SymbolEqualityComparer.Default)) {
				operationContext.ReportDiagnostic (Diagnostic.Create (GeneratedDiagnostics.GetDiagnostic ("IL3000"),
				...
				operationContext.ReportDiagnostic (Diagnostic.Create (GeneratedDiagnostics.GetDiagnostic ("IL3001"),
					operationContext.Operation.Syntax.GetLocation (), member.GetDisplayName ()));
